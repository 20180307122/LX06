...
namespace DAL
{
    public class Comment
    {
        ...
        public int GetWorkCount(int id)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select count(1) from Comment where wordId=@wordId";
                return cn.ExecuteScalar<int>(sql,new { wordId = id });
            }
        }
        ...
        public int Add(Model.Comment comment)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "insert into Comment values(commentID ,workId,userName,CommentContent,CommentTime)"+"" +
                    "valuse(@commentID ,@workId,@userName,@CommentContent,@CommentTime);";
                sql += "SELECT @@IDENTITY";
